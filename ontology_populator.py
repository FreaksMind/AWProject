...
class OntologyPopulator:
    ...
    def add_dbpedia_information(self):
        """Add programming languages, paradigms, and operating systems data from DBpedia to the ontology."""
    ...
    def populate_ontology(self):
        print("Adding DBpedia information for programming languages, paradigms, and operating systems...")
        self.add_dbpedia_information()
        print("Done adding DBpedia information.")

        # GitHub data population
        predefined_topics = {
            "framework": "Framework",
        ...
        for key, topic in predefined_topics.items():
            print(f"Fetching repositories for predefined topic '{key}'...")
        ...
        # add user custom topic
        custom_topic = input("Enter a custom topic to fetch (e.g., 'cloud-based platform'): ").strip().lower()
        print(f"Fetching repositories for custom topic '{custom_topic}'...")
        repos = self.fetch_repos_by_topic(custom_topic, min_stars=1000, limit=50)
        self.insert_repos_into_ontology(repos, custom_topic)
        print(f"Done processing '{custom_topic}' topic.\n")

        new_version = self.file_version + 1
        self.g.serialize(destination=f"v{new_version}.owl", format="xml")
        print(f"Ontology updated and saved as v{new_version}.owl")
